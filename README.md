...
> 🎨 **Прототип лендинга** (маркетинговая обложка): <https://chitai.bolt.host/#>
> Здесь же — рабочий backend + UI: вкладка «Учёба» (`/api/study/*`), тренажёр ЕГЭ (131 вопрос с пояснением каждого варианта), куратор, ментальная карта, игра Brain Dash.
...
2. **«Учёба»** — загрузка PDF / аудио / видео / URL / текста → AI-конспект (summary + key_moments + tips + glossary), Q&A 24/7, флэшкарты, Smart-Quiz с per-option объяснениями, fill-in-the-blank, эссе с оценкой по рубрике 5×5, экспорт в HTML/PDF, share-токены, mastery-прогресс. Бэкенд готов: `POST /api/study/ingest` + 20 endpoints, тарифы Free / Неделя / Месяц / Год.
